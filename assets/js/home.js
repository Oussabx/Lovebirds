...
(function () {
  const { $, ICONS, productCard, observe, refreshParallax, esc, safeUrl } = window.LB;
  ...
  function render() {
    ...
    if (cats) {
      cats.innerHTML = CATEGORIES.map(c => [
        '<a class="cat-card" href="categories.html?cat=' + encodeURIComponent(c.id) + '">',
        '<div class="cat-card__media"><img src="' + esc(safeUrl(c.image)) + '" alt="' + esc(c.name) + '" loading="lazy" width="640" height="640"></div>',
        '<div class="cat-card__body"><h3>' + esc(c.name) + '</h3><p>' + esc(c.blurb || '') + '</p></div>',
        '</a>'
      ].join('')).join('');
    }
    ...
    if (marquee) {
      const items = (home.marquee || []).filter(text => String(text || '').trim()).map(text =>
        '<span>' + esc(text) + '</span>' + ICONS.heart).join('');
    ...
    if (storyImages && home.story) {
      ...
      storyImages.innerHTML = images.map((src, i) =>
        '<img src="' + esc(safeUrl(src)) + '" alt="" width="640" height="640" loading="lazy" data-parallax="' + (i % 2 ? '-0.04' : '0.04') + '">'
      ).join('') + (home.story.note
        ? '<span class="story__note">' + esc(home.story.note).replace(/\n/g, '<br>') + '</span>' : '');
    }

    const points = $('[data-story-points]');
    if (points && home.story) {
      points.innerHTML = (home.story.points || []).filter(p => p && String(p.text || '').trim()).map(p =>
        '<li>' + icon(p.icon) + ' ' + esc(p.text) + '</li>').join('');
    }
    ...
    if (values && home.values) {
      values.innerHTML = (home.values.items || []).filter(v => v && (String(v.title || '').trim() || String(v.text || '').trim())).map(v => [
        ...
        '<h3>' + esc(v.title) + '</h3><p>' + esc(v.text) + '</p></div>'
      ].join('')).join('');
    }
    ...
    if (quotes && home.quotes) {
      quotes.innerHTML = (home.quotes.items || []).filter(q => q && String(q.text || '').trim()).map(q => [
        ...
        '</div><p>“' + esc(q.text) + '”</p><footer>' + esc(q.author) + '</footer></blockquote>'
      ].join('')).join('');
    }
